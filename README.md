...
calling the `GenerateClientSecret` function included or `GenerateClientSecretP8File` function for use p8 file directly. Check [secret.go](apple/secret.go) to see exactly how to obtain the 
...
To use `GenerateClientSecretP8File`
```
import "github.com/Timothylock/go-signin-with-apple/apple"
...
p8FilePath := "./example/.secret_key.p8"

secret, _ := apples.GenerateClientSecretP8File(p8FilePath, team_id, client_id, key_id)
fmt.Println(secret)
```
